...
export default class Draw {
  ...
  addImage(param) {
    var p = this.downloadImage(param.src);
    if (param.defealtSrc) {
      let defealtSrc = param.defealtSrc;
      p = p.catch(e => {
        return this.downloadImage(defealtSrc)
      })
    }
    param.type = "image";
  ...
  addCustom(callback, src) {
    ...
    if (src) {
      if (typeof src === "string") {
        this.preLoads.push(this.downloadImage(src));
        param.len = 1;
      } else {
        src.forEach(item => {
          this.preLoads.push(this.downloadImage(item));
        })
  ...
  drawImage(param) {
    ...
    if (radiu == 0) {
      ...
      return ctx.drawImage(src, sx, sy, sw, sh, dx, dy, dw, dh);
    }
    ctx.save()
  ...
  downloadImage(url) {//图片下载获得临时地址promise
    if (!url) return Promise.resolve(null);
    if (url.indexOf('//tmp') === -1) url = url.replace(/https?/, 'https');
    return new Promise((resolve, reject) => {
      wx.getImageInfo({
        src: url,
        success: resolve,
